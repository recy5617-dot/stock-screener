...
4. 右邊會有「Run workflow」按鈕，先手動點一次測試（不用等到排程時間），確認能正常跑。
   按下去後可以填日期、也可以用下拉選單選「兩種都跑／只跑當沖／只跑波段(原本的選股)」；每天的自動排程固定是兩種都跑。
...
## ⚡ 隔日當沖候選名單（第二套選股，預設跟波段選股一起跑）

上面的 7 條件是「波段」邏輯（趨勢會不會延續幾天）。當沖要的東西不一樣：
**流動性夠（進出不卡）＋ 波動夠大（有價差）＋ 有人氣（量能放大）＋ 方向清楚（收盤強/弱、順勢）**，
所以另外寫了一套 `daytrade.py`，收盤後挑出「明天」值得盯盤的當沖標的。

**硬性濾網**（不符合直接不列入）：成交量 ≥ 3000 張、成交值 ≥ 3 億、股價 10~1000 元、排除 00 開頭 ETF、
不在官方「可當沖標的清單」上的排除（有抓到清單時才套用）。

**6 個計分項目**（總分 100，≥60 分列入，最多 30 檔）：

| 項目 | 權重 | 做多看什麼 | 做空看什麼 |
|---|---|---|---|
| A 波動度 | 25 | ATR% 在 2.5%~8%（太小賺不到價差，太大停損易被掃） | 同左 |
| B 流動性 | 20 | 成交值越大越好（3億→30億 對數給分） | 同左 |
| C 量能 | 20 | 今日量 / 5日均量 ≥ 1.5 倍（2.5 倍滿分） | 同左 |
| D 收盤 | 15 | 收在當日振幅上緣 30% | 收在下緣 30% |
| E 順勢 | 10 | 收盤 > 5日線 > 10日線，或突破 20 日高 | 反之，或跌破 20 日低 |
| F 法人 | 10 | 外資或投信買超 | 外資或投信賣超 |

- **方向**：今天收紅列「偏多」、收黑列「偏空」。官方註記「暫停先賣後買」的股票不會出現在偏空名單。
- **漲跌停附近**（±9.5%）扣 10 分並加註：明天常直接跳空，不好進場。
- 每檔附上明天的**參考價位**：今日高低點、Pivot / R1 / S1、ATR 與參考停損距離（ATR×0.5），方便盤前先標好關鍵價。
- 輸出：`output/daytrade_YYYYMMDD.csv`、網頁 `docs/daytrade.html`（波段報表頁上方有連結可切換）。

```bash
python main.py                      # 波段 + 當沖 兩套都跑（預設）
python main.py --daytrade-only      # 只跑當沖名單
python main.py --no-daytrade        # 只跑波段
python main.py --dt-min-score 70    # 當沖名單只列 70 分以上
python main.py --test-daytrade      # 測試官方可當沖清單(TWTB4U)端點
python test_daytrade.py             # 當沖邏輯自我測試（不需網路）
```

所有門檻與權重都在 `config.py` 的 `DT_` 開頭參數（例如資金較小可以把 `DT_MAX_PRICE` 調低）。

> ⚠️ 可當沖清單（TWSE TWTB4U）接口在開發環境連不到、**未經實測**，程式用欄位名稱比對解析；
> 抓不到或格式不符時**不會**套用這層濾網（名單照出，報表會標示「未取得官方可當沖清單」），
> 可先跑 `python main.py --test-daytrade` 確認。
>
> 限制：這裡只有**日K**資料，看不到盤中分時、委買賣、隔日開盤跳空，名單只是「縮小盤前觀察範圍」，
> 真正進場要看開盤後的量價並嚴守停損。當沖虧損要自行負擔價差＋手續費＋證交稅，資金不足還有違約交割的法律風險。
...
每次執行會自動整理快取：只保留最近約 80 個交易日、刪掉權證等用不到的法人資料，
讓檔案維持在 20MB 左右（GitHub 單檔上限 100MB，超過會讓自動排程 push 失敗）。

沒指定 `--date`、而今天收盤資料還沒公告（例如下午2點半前手動執行）時，會自動改跑快取裡最近一個交易日。
