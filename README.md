...
依照你設定的邏輯做的收盤後選股程式，現在共 **7 項條件**：

1. **股價站上 20日月線**（最好月線走平或向上；剛站回月線也列入觀察）
2. **KD 向上打勾**（K由下往上轉，最漂亮是 K上穿D；高檔鈍化已久的打勾會降權）
3. **法人籌碼轉強**（預設較嚴格版本：**外資當天淨買超 且 投信連買3天以上**，兩者要同時成立；
   可在 `config.py` 用 `CHIPS_REQUIRE_FOREIGN_AND_TRUST=False` 切回原本「外資由賣轉買 / 投信連買 / 三大法人合計由賣轉買，任一即算」的寬鬆版本）
4. **融資沒有暴增**（股價漲＋融資也暴增 -> 不算過關；融資持平或下降更漂亮）
5. **突破或轉強有量**（帶量突破前高/整理區，且收盤不是長上影線；成交量低於 `MIN_VOLUME_LOTS`(預設1000張) 的股票直接不列入報表）
6. **動能指標同時成立**：MACD 柱狀圖翻紅 ＋ RSI 落在設定區間(預設50~70，避免追高) ＋ 5日線在10日線之上
7. **布林通道**：收盤價站上布林通道上軌，且上軌本身向上（代表波動擴張、多頭延續）

每檔股票會算出：① ~ ⑦ 是否過關（達成數 0~7，對應「7項全中→🔥主力觀察名單／達成≥5→值得研究／達成≥3→等待確認／2項以下→跳過」，門檻可在 `config.py` 的 `TIER_FULL_MIN/TIER_GOOD_MIN/TIER_WATCH_MIN` 調整），
以及一個 0~100 的**加權分數**，權重依優先順序：**月線(30) ＞ 籌碼(25) ＞ 成交量(20) ＞ KD(10) ≈ 動能(10) ＞ 布林(5)**，融資暴增則直接扣分、融資乾淨則小幅加分。
同一達成數的股票，會再用加權分數排序。

另外還有一個**選用、預設關閉**的加分參考：大戶持股比例變化（集保中心週資料），看下面「可以調整的地方」。
...
代號    名稱     收盤    漲跌%  ①月線 ②KD  ③籌碼 ④融資 ⑤量  ⑥動能 ⑦布林 達成 分級        加權分  備註
2330   台積電   1150.00 +2.15% ✅   ✅   ✅   ✅   ✅   ✅   ✅   7   🔥主力觀察名單 96.0   站穩月線且月線向上；K上穿D...
```
...
python main.py --min 5            # 只看達成5項以上的（預設3項，總共7項）
```
...
- `WEIGHT_MA20 / WEIGHT_CHIPS / WEIGHT_VOLUME / WEIGHT_KD / WEIGHT_MOMENTUM / WEIGHT_BOLLINGER`：加權分數的權重比例
- `MARGIN_SURGE_THRESHOLD`：融資「暴增」的門檻（預設漲超過3%算暴增）
- `VOLUME_SURGE_RATIO`：「放量」的倍數門檻（預設要大於近5日均量的1.5倍）
- `MIN_VOLUME_LOTS`：成交量的硬性流動性門檻（預設1000張，低於這個量直接不列入報表）
- `KD_HIGH_ZONE` / `KD_STAGNANT_DAYS`：判斷「高檔鈍化太久」的標準
- `BREAKOUT_LOOKBACK`：判斷「前高/整理區」回顧幾天
- `CHIPS_REQUIRE_FOREIGN_AND_TRUST`：③籌碼條件要不要用較嚴格的「外資+投信同時買超」版本（預設True）；
  改成 `False` 會切回原本「外資由賣轉買 / 投信連買 / 三大法人合計由賣轉買，任一即算」
- `TRUST_CONSEC_BUY_DAYS`：投信「連買」至少要幾天才算數（預設3天）
- `MACD_FAST/MACD_SLOW/MACD_SIGNAL/MACD_TURN_LOOKBACK`：⑥MACD相關參數（標準值12/26/9）
- `RSI_PERIOD/RSI_MIN/RSI_MAX`：⑥RSI週期與「動能區間」範圍（預設14週期、50~70區間，避免追高）
- `MA_FAST/MA_MED`：⑥判斷「5日線穿10日線」用的兩條均線天數
- `BB_PERIOD/BB_STD/BB_SLOPE_LOOKBACK/BB_TOUCH_TOLERANCE`：⑦布林通道參數（標準值20週期、2倍標準差）
- `TIER_FULL_MIN/TIER_GOOD_MIN/TIER_WATCH_MIN`：達成數對應到「主力觀察名單／值得研究／等待確認」的門檻

### 選用功能：大戶持股比例變化（預設關閉）

資料來源是集保結算所(TDCC)「集保股權分散表」的公開資料，**一週才更新一次**（通常每週五公告），
不是每天都有新資料，且「大戶」的持股級距門檻（`BIG_HOLDER_MIN_TIER`，預設12，約400張以上）是常見慣例值，
不是官方正式定義。啟用前建議先手動確認能抓到資料：

```bash
python main.py --test-big-holder
```

看到範例比例數字合理之後，把 `config.py` 的 `ENABLE_BIG_HOLDER_CHECK` 改成 `True`。
啟用後，這項資料只會拿來當③籌碼條件的「加分參考」（大戶持股比例比前一期上升會小幅加分），
不會影響③是否過關，避免因為週資料偶爾缺值而誤判。
...
fetch_big_holder.py 大戶持股比例抓取（選用功能，預設關閉，見檔頭說明）
